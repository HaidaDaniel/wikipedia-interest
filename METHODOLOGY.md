...
Charts and PDF reports select an installed font by checking coverage for common Latin, accented Latin, Cyrillic, Japanese and Korean characters. No font files are bundled. If the host has no single font covering the requested scripts, rendering continues with the best available fallback and emits a warning; installing Noto Sans CJK, Source Han Sans, WenQuanYi or Unifont provides broader coverage.
